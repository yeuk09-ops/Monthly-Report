...
### 2.1 사용자 제공 CSV (월별 재무제표만)

| 데이터 | 파일명 | 업데이트 위치 | 비고 |
|:---|:---|:---|:---|
| **월별 재무제표** | `F&F 월별재무제표(YY.MM).csv` | `financialData` 객체 | **유일한 CSV 입력** |

**CSV에서 가져오는 항목:**
- 손익계산서 주요 항목: 실판매출, 매출원가, 판관비, 영업이익
- 재무상태표 주요 항목: 현금, 매출채권, 재고자산, 총자산, 총부채, 자본

### 2.2 Snowflake 직접 검색 데이터 (대부분)

| 데이터 | 테이블 | 업데이트 위치 | 쿼리 파일 |
|:---|:---|:---|:---|
| **브랜드별 매출** | `DW_COPA_D` | `income-statement` 브랜드별 실적 | `fnf_sales_guide_v7.md` 참고 |
| **채널별 매출** | `DW_COPA_D` | `income-statement` 채널별 매출 | `fnf_sales_guide_v7.md` 참고 |
...
| **재고자산 (브랜드별)** | `DW_IVTR_HIST` | `workingCapitalInv` 배열 | `inventory_verify.py` |
...
WHERE PST_DT >= '2025-01-01' AND PST_DT < '2026-01-01'
  AND CORP_CD = '1000'
  AND CHNL_CD IN ('1', '2', '3', '5', '11')  -- 국내 채널
GROUP BY CHNL_CD;
```
...
> 상세 내용: `Monthly_Report_Guides/FNF_손익CO_데이터_가이드.md` 참조
...
│                                                                     │
│  【Snowflake COPA 컬럼】                                            │
│  ├── ACT_SALE_AMT: 실판매액 (V+, VAT 포함)                          │
│  ├── VAT_EXC_ACT_SALE_AMT: 실판매액 (V-, VAT 제외)                  │
│  ├── SUPP_AMT: 출고매출액 (V+)                                       │
│  └── ACT_COGS: 매출원가                                             │
│                                                                     │
│  【국내 vs 수출】                                                    │
│  ├── 국내매출: V+ → V- 변환 필수 (÷ 1.1)                            │
│  └── 수출매출: 영세율이므로 V+ = V- (변환 불필요)                   │
└─────────────────────────────────────────────────────────────────────┘

【영업이익 계산 공식】
1. 재무매출(V-) = 국내 ACT_SALE_AMT / 1.1 + 수출 ACT_SALE_AMT
2. 점수수료(V-) = {ACT_SALE_AMT - SUPP_AMT} / 1.1
3. 출고매출(V-) = 재무매출 - 점수수료
4. 영업이익 = 재무매출 - 점수수료 - 매출원가 - 판관비

【대시보드 표시】
- 실판매출(V-): VAT 제외 금액으로 표시
...
- 브랜드별 매출/영업이익
- 채널별 매출 (국내: 백화점/대리점/온라인, 수출: 중국/홍콩/기타)
...
| 문서 | 위치 | 용도 |
|:---|:---|:---|
| **손익(CO) 데이터 가이드** | `Monthly_Report_Guides/FNF_손익CO_데이터_가이드.md` | **손익계산서 핵심 - COPA 쿼리, 영업이익 계산** |
...
| | | 브랜드별/채널별 매출 쿼리 추가 |
| 2025-01-15 | 2.1 | 손익CO 가이드 통합 (FNF_손익CO_데이터_가이드.md 참조 추가) |
| | | 영업이익 계산 로직 추가 (V+/V- 구분, 국내/수출 VAT 처리) |

---

*Generated by Claude - FNF Dashboard Monthly Update Guide v2.1*
